...
import { Linking, Pressable, SafeAreaView, ScrollView, Text, TextInput, View } from "react-native";
...
const botReplies: Record<string, string> = {
  "How do I book a service?": "Go to Home, choose a service category, fill details, then confirm your booking.",
  "How do I track my request?": "Open Hire/Activity to see request status, provider updates, and current progress.",
  "How do I make payment?": "You can pay during booking with available payment options and view receipts in your wallet/history.",
  "How do I cancel a request?": "Open the active request and tap cancel. Follow the prompts to confirm cancellation.",
  "How do I update my profile?": "Go to Profile, open Edit Profile, update your details, and save.",
};
...
export default function SupportChatScreen() {
  ...
  const handleQuickReply = (question: string) => {
    const userMessage: ChatMessage = { id: `${Date.now()}-u`, text: question, from: "user" };
              ...
              <View key={item.id} className={item.from === "user" ? "items-end" : "items-start"}>
                <View
                  className={`max-w-[85%] rounded-2xl px-3 py-2 ${
                    item.from === "user" ? "bg-[#EAF9F1]" : "border border-[#E5E7EB] bg-white"
                  }`}
                >
